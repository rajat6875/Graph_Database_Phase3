...
public class TupleUtils
{
  ...
  public static int CompareTupleWithTuple(AttrType fldType,
	...
	case AttrType.attrDesc:
		try{
			//t1_d = t1.get(t1_fld_no);
			//t2_d = t2.get(t2_fld_no);
			t1_d = t1.getDescFld(t1_fld_no);
			t2_d = t2.getDescFld(t2_fld_no);
			
			double distBtwT1T2 = t1_d.distance(t2_d);
			double distBtwT1Target = t1_d.distance(target);
			double distBtwT2Target = t2_d.distance(target);
			...
			{
				if(distBtwT1T2 <= distance) return 0;
				else return 1;
			}
			else
			{
				if(distBtwT1Target == distBtwT2Target) return  0;
				if(distBtwT1Target <  distBtwT2Target) return  1;
				if(distBtwT1Target >  distBtwT2Target) return -1;
			}
		}
		catch (FieldNumberOutOfBoundException e){
  ...
  public static boolean Equal(Tuple t1, Tuple t2,
		  				AttrType types[], int len,
		  				double distance, Descriptor target)
    throws IOException,UnknowAttrType,TupleUtilsException
    {
      ...
      for (i = 1; i <= len; i++)
    	  if (!types[i-1].toString().equals("attrDesc"))
    	  {
    	  ...
    	  else
    	  {
    		  try
    		  {
    			  Descriptor t1_d = t1.getDescFld(i-1);
        		  Descriptor t2_d = t2.getDescFld(i-1);
        		  ...
        		  if(target.equals(temp))
        		  {
        			  if(distBtwT1T2 <= distance) return true;
        			  else return false;
        		  }
        		  else
        		  {
        			  if(distBtwT1Target == distBtwT2Target) return true;
        			  else return false;
        		  } 
    		  }
    		  catch (FieldNumberOutOfBoundException e)
